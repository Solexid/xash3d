...
void CL_UpdateUserinfo( sizebuf_t *msg )
{
	...
	if( active )
	{
		...
		if( slot == cl.playernum ) Q_memcpy( &gameui.playerinfo, player, sizeof( player_info_t ));
	}
...
int CL_ParseScreenShake( const char *pszName, int iSize, void *pbuf )
{
	...
	clgame.shake.amplitude = (float)(word)MSG_ReadShort( msg ) * (1.0f / (float)(1U << 12));
	clgame.shake.duration = (float)(word)MSG_ReadShort( msg ) * (1.0f / (float)(1U << 12));
	clgame.shake.frequency = (float)(word)MSG_ReadShort( msg ) * (1.0f / (float)(1U << 8));
...
int CL_ParseScreenFade( const char *pszName, int iSize, void *pbuf )
{
	...
	duration = (float)(word)MSG_ReadShort( msg );
	holdTime = (float)(word)MSG_ReadShort( msg );
...
void CL_ParseServerMessage( sizebuf_t *msg )
{
	...
	while( 1 )
	{
		...
		cl.frames[cl.parsecountmod].graphdata.msgbytes += MSG_GetNumBytesRead( msg ) - bufStart;
	}
