...
void UI_UnloadProgs( void )
{
	...
	Q_memset( &gameui, 0, sizeof( gameui ));
}
...
#if TARGET_OS_IPHONE
	if(!( gameui.hInstance = Com_LoadLibrary( "menu", false )))
		return false;
#elif defined (__ANDROID__)
	char menulib[256];
	Q_snprintf( menulib, 256, "%s/%s", getenv("XASH3D_GAMELIBDIR"), MENUDLL );
	if(!( gameui.hInstance = Com_LoadLibrary( menulib, false )))
	{
...
#else
	if(!( gameui.hInstance = Com_LoadLibrary( va( "%s/" MENUDLL, GI->dll_path ), false )))
	{
		FS_AllowDirectPaths( true );

		if(!( gameui.hInstance = Com_LoadLibrary( "../" MENUDLL, false ))
				&& !( gameui.hInstance = Com_LoadLibrary( MENUDLL, false )))
