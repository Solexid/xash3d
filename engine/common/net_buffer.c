...
void MSG_WriteUBitLongExt( sizebuf_t *sb, uint curData, int numbits, qboolean bCheckRange )
{
...
void MSG_WriteSBitLong( sizebuf_t *sb, int data, int numbits )
{
	...
	if( data < 0 )
	{
		MSG_WriteUBitLongExt( sb, (uint)( 0x80000000 + data ), numbits - 1, false );
...
qboolean MSG_WriteBits( sizebuf_t *sb, const void *pData, int nBits )
{
	...
	{
		MSG_WriteUBitLongExt( sb, *pOut, 8, false );
	...
	while( nBitsLeft >= 32 )
	{
		MSG_WriteUBitLongExt( sb, *(( dword *)pOut ), 32, false );
	...
	if( nBitsLeft )
	{
		MSG_WriteUBitLongExt( sb, *pOut, nBitsLeft, false );
	}
