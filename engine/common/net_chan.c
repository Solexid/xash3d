...
qboolean Netchan_CopyFileFragments( netchan_t *chan, sizebuf_t *msg )
{
	...
	Q_snprintf( filename, sizeof( filename ), "downloaded/%s", MSG_ReadString( msg ) );
...
void Netchan_TransmitBits( netchan_t *chan, int length, byte *data )
{
	...
	if( chan->sock == NS_CLIENT )
	{
		MSG_WriteWord( &send, net_qport->integer);
	}	
	...
	if( !CL_IsPlaybackDemo( ))
	{
		unsigned int size = MSG_GetNumBytesWritten( &send );

		if( chan->split && chan->remote_address.type != NA_LOOPBACK && size > chan->maxpacket )
			NetSplit_SendLong( chan->sock, size, MSG_GetData( &send ), chan->remote_address, chan->maxpacket, chan->splitid++ );
		else
			NET_SendPacket( chan->sock, size, MSG_GetData( &send ), chan->remote_address );
	}
