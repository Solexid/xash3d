...
=============================================================================
*/
int matchpattern( const char *str, const char *pattern, qboolean caseinsensitive )
{
	return matchpattern_with_separator( str, pattern, caseinsensitive, "/\\:", false );
}
...
void FS_Rescan( void )
{
	...
	if( str = getenv("XASH3D_EXTRAS_PAK1") )
		FS_AddPak_Fullpath( str, NULL, false, FS_NOWRITE_PATH | FS_CUSTOM_PATH );
	...
	{
		FS_AddPak_Fullpath( va( "%sextras.pak", SDL_GetBasePath() ), NULL, false, FS_NOWRITE_PATH | FS_CUSTOM_PATH );
		FS_AddPak_Fullpath( va( "%sextras_%s.pak", SDL_GetBasePath(), GI->gamedir ), NULL, false, FS_NOWRITE_PATH | FS_CUSTOM_PATH );
	}
#endif
...
searchpath_t *FS_FindFile( const char *name, int* index, qboolean gamedironly )
{
	...
	for( search = fs_searchpaths; search; search = search->next )
	{
		if( gamedironly && !FBitSet( search->flags, FS_GAMEDIR_PATH | FS_CUSTOM_PATH ))
			continue;
...
search_t *FS_Search( const char *pattern, int caseinsensitive, int gamedironly )
{
	...
	{	
		if( gamedironly && !FBitSet( searchpath->flags, FS_GAMEDIR_PATH | FS_CUSTOM_PATH ))
			continue;
		...
		else if( searchpath->wad )
		{
			...
			for( i = 0; i < wad->numlumps; i++ )
			{
				...
				Q_snprintf( temp, sizeof( temp ),
					"%s%s", wad->lumps[i].name, wad_hints[wad->lumps[i].img_type].ext);
				while( temp[0] )
				{
...
byte *W_ReadLump( wfile_t *wad, dlumpinfo_t *lump, long *lumpsizeptr )
{
	...
	oldpos = tell( wad->handle ); // don't forget to restore original position
...
wfile_t *W_Open( const char *filename, const char *mode, int *error )
{
	...
	if( wad->handle < 0 )
	{
		...
		{
			wad->handle = open( ffilename, mod|opt, 0666 );
		}
	}
#endif
...
size_t W_SaveFile( wfile_t *wad, const char *lump, const void *data, size_t datasize, char type, qboolean replace )
{
	...
	Q_memset( &newlump, 0, sizeof( newlump ));
